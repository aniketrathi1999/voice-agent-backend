...
from livekit.plugins import deepgram, noise_cancellation, openai, silero, assemblyai
...
EOT_CONF_THRESH = float(os.getenv("EOT_CONF_THRESH", "0.9"))
MIN_EOT_SILENCE_MS = int(os.getenv("MIN_EOT_SILENCE_MS", "100"))
MAX_TURN_SILENCE_MS = int(os.getenv("MAX_TURN_SILENCE_MS", "1000"))

class BaselineAgent(Agent):
    """Agent with configured STT/LLM/TTS backends."""

    def __init__(self) -> None:
        print("STT_PROVIDER", STT_PROVIDER)
        if STT_PROVIDER == "deepgram":
            ...
            stt = assemblyai.STT(
                end_of_turn_confidence_threshold=EOT_CONF_THRESH,
    ...
    async def start_turn_trace():
        ...
        async def span_runner():
            ...
            span.update_trace(
                ...
                tags=["experiment2", "voice", "per_turn"],
                metadata={
                    ...
                    "allow_interruptions": ALLOW_INTERRUPTIONS,
                    "preemptive_generation": PREEMPTIVE_GENERATION,
                    "eot_conf_thresh": EOT_CONF_THRESH,
                    "min_eot_silence_ms": MIN_EOT_SILENCE_MS,
